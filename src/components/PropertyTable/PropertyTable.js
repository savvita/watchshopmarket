...
import { selectValues, selectStatus } from '../../app/waterresistanceSlice';
            ...
            <Table dark hover className="property-table__table table_sort">
                ...
                <tbody>
                    ...
                    { itemsPage && itemsPage.map((item, idx) => 
                        item && <PropertyTableRow key={ item.id } idx={ idx + 1 } item={ item } onDelete={ (item) => showCofirmModal(item) } onAccept={ saveItem } onError={ onError } />)
                    }
                </tbody>
                <tfoot>
                    <tr>
