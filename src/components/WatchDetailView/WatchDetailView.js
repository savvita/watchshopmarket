...
import { selectStatus as selectWatchStatus, selectCurrent as selectValue, getByIdAsync } from '../../app/watchSlice';
import { selectStatus as selectOrderStatus, getByFiltersAsync } from '../../app/orderSlice';

const WatchDetailView = ({ id }) => {
    const value = useSelector(selectValue);
    const [orders, setOrders] = useState([]);
    const status = useSelector(selectWatchStatus);
    ...
    const getSales = async () => {
        ...
        const res = await dispatch(getByFiltersAsync({ watchId: id }));
        
        if(res && res.payload && res.payload.value) {
            setOrders([...res.payload.value]);
            let total_s = 0;
            const month_s = [];
            const month_c = [];
            ...
            for(let order of res.payload.value) {
                if(!order.details) {
                    continue;
                }
    ...
    return (
        <div className="text-white mt-3">
            ...
            <div className={ status === 'idle' ? 'p-2 mt-3' : 'd-none' }>
                { value &&  <h3>{ value && value.title }</h3>}
            </div>          
